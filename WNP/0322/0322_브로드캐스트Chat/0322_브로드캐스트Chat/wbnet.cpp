...
void wbnet_SendData(const char* buf, int gruopid, int port)
{
		SOCKADDR_IN addr;
		addr.sin_family = AF_INET;
		addr.sin_port = htons(port);
		addr.sin_addr.s_addr = inet_addr(g_multiIP[gruopid]);

		sendto(g_sendSocket, buf, sizeof(TCHAR) * 200, 0, (SOCKADDR*)&addr, sizeof(addr));
}