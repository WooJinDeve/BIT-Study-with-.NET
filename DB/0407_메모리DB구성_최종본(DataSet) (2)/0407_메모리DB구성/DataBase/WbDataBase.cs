...
namespace _0407_메모리DB구성
{
    internal class WbDataBase
    {
        ...
        private void MemberAdd(string name, string phone)
        {
            try
            {
                DataRow dr = ds.Tables["Member"].NewRow();
                dr["name"] = name;
                dr["phone"] = phone;
        ...
        {
            DataTable acc = ds.Tables["Account"];
            int accid = int.Parse(acc.Rows[acc.Rows.Count - 1]["accid"].ToString());
            return accid;
        }
        ...
        private void AccountioAdd(int accid, int input, int output,  int balance)
        {
            try
            {
                DataRow dr = ds.Tables["Accountio"].NewRow();
                dr["accid"] = accid;
                dr["input"] = input;
                dr["output"] = output;
                dr["balance"] = balance;
                dr["newtime"] = DateTime.Now;
                ds.Tables["Accountio"].Rows.Add(dr);
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
        ...
        private void PrintAll(DataTable dt)
        {
            ...
            {
                //만약 컬럼을 삭제하면 해당 컬럼은 delete로 처리되고, 
                //해당 부분을 접근하여 출력하면 예외가 발생됨
                try
                {
        ...
        public void InputMoney()
        {
            try
            {
                ...
                ds.Tables["Accountio"].AcceptChanges();
            }
            catch(Exception)
            {
                ds.Tables["Account"].RejectChanges();
                ds.Tables["Accountio"].RejectChanges();
            }
        }
        ...
        {
            DataRow row = ds.Tables["Accountio"].NewRow();
            ...
            ds.Tables["Accountio"].Rows.Add(row);
        }
        ...
        private void AccoutIODelete(int accid)
        {
            ...
            //컬럼 삭제는 반복문에서 연속으로 삭제 할 수 없다.
            //삭제하는 순간 테이블의 컬럼 정보가 변경되어 오류 발생
            //DataTable dt = ds.Tables["Accountio"];
        ...
        private void AccountDelete(int accid)
        {
            ...
            DataRow row = dt.Rows.Find(accid);
            dt.Rows.Remove(row);
        }
