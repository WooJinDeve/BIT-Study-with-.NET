...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		g_listbox1 = CreateWindow(_T("listbox"), NULL, WS_CHILD | WS_BORDER | WS_VISIBLE | LBS_NOTIFY
			, 20, 50, 200, 300, hwnd, (HMENU)IDC_LISTBOX1, GetModuleHandle(0), 0);

		//------------- 리스트 박스에 원하는 문자열 저장 ---------------
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)_T("첫번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)_T("두번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)_T("세번째문자열"));
		SendMessage(g_listbox1, LB_ADDSTRING, 0, (LPARAM)_T("네번째문자열"));
